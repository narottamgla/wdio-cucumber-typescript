...
class LoginPage extends Page {
    ...
    get signOutLink() { return $("(//a[@href='/authentication/logout/'])[1]") }
    ...
    async login(username: string, password: string) {
        await this.iframeId.waitForDisplayed({ timeout: 30000, timeoutMsg: "Timeout error: Not able to switch to Iframe" });
        browser.switchToFrame(0).then(async () => {
            await browser.pause(2);
            await this.inputUsername.waitForDisplayed({ timeout: 30000, timeoutMsg: "Timeout error: element not displayed" });
    ...
    async validateLogin() {
        ...
        await this.signOutLink.waitForDisplayed({ timeout: 60000 });
    ...
    async performLogout() {
        browser.switchToParentFrame().then(async () => {
